...
export async function fetchAllCompanies(): Promise<{ id: number }[]> {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/companies/ids`);
...
export async function fetchCompanyDetail(id: string): Promise<Company> {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/companies/${id}`);
    return response.data.data;
  } catch (error) {
